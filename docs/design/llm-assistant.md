...
Status: **DRAFT awaiting Marc's sign-off.** Milestone **M14 Assistant (local LLM)** (umbrella issue
and slices A–I in §10; the numbers are filled in once the issues exist).
...
answer. The ones that need more than a yes are also filed as `question` issues.
