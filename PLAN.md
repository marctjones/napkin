...
are features, not legacy; they simply arrive one at a time (M4 ships Connecticut 2022 — IRC 2021
as amended; the 2026 code is not yet in force — M10 makes its tables real, and Massachusetts and
Pennsylvania come after that). Nothing is scheduled toward a date. Versioning is decided
(DESIGN.md §12, #30): the minor number is bumped by whoever lands each change on `main` (no pull
requests since 2026-09-22; CLAUDE.md) and a release is tagged when features improve significantly.
...
| **M4 Check** | Draw an exterior wall, enter what it supports and the site hazard values yourself, place a window on it, and see a header size and stud count **with the code edition, table and row it came from**. Resize the opening and the answer follows. Push it past what the table covers and napkin says so and stops, citing the limit. **One** adopted code pack — Connecticut 2022 (IRC 2021 as amended); the 2026 code is not yet in force — and the per-project picker that selects it; the other three packs come later. Designed in [`docs/design/rules-engine-model.md`](./docs/design/rules-engine-model.md). Done (0.97.0-beta, 2026-09-25): engine, pack format, CT overlay, interpolation and the picker are complete; the IRC 2021 base tables are bring-your-own until #158/#14 transcribe them (Marc decided on 2026-09-25 that transcribed tables may ship when read from a primary source). | #12, #13, #14, #18, #19 (the picker) |
| **M5 Brace and compare** | **Done as a mechanism (0.101.0-beta, 2026-09-25), proven with synthetic packs; real R602.10 tables are a Backlog issue.** Enter the bracing that already exists along a wall line, widen an opening step by step, and watch napkin flag the wall when the line runs short — naming the section and the shortfall. Then switch the project to a second pack (Connecticut 2026 once adopted, or another jurisdiction) and watch every result recompute, with anything that no longer holds re-flagged instead of quietly carrying over. Two packs is what turns code locking from an assertion into something you can see happen. | #15, #19 (recompute), #39 |
| **Hardening** | Not a feature milestone: a pass fixing architecture drift, correctness risks and stale docs found by a full-repo review (2026-09-25), before new feature work resumes. The local landing scripts (`tools/scripts/gate.sh`, `land.sh`, `gui-ratchet.sh`, `mutate.sh`), `.gitattributes` line-ending normalization, the unsized-header and shank-diameter exactness fixes, narrowed file-open exception filters, removing the parked Interop stubs, and this doc sweep. | #165, #3, #171, #173, #175, #182, #177, #121 |
| **M6 Views and drawings** | Look at the coffee table from all six sides in a locked orthographic view — hidden edges dashed, dimensions and rulers per view — lay the views out as a third-angle sheet, open a Parts view that shows every distinct piece once with its count, and save the sheet as a PDF. Designed in [`docs/design/standard-views.md`](./docs/design/standard-views.md) and [`docs/design/parts-view.md`](./docs/design/parts-view.md); Marc accepted their defaults on 2026-09-23 (#116). Slice A landed 0.102.0-beta. | #107, #128, #129, #130, #131, #133, #134, #122–#127, #156, #25 (sheet only) |
| **M7 Sketch mode** | A Rough/Precise sketch mode: draw loosely first, without committing to exact dimensions, then firm a sketch up into the exact-length model the rest of napkin depends on. Designed in [`docs/design/sketch-mode.md`](./docs/design/sketch-mode.md). | design note landed; implementation not started |
...
- **Test infrastructure**: the coverage ratchet (#32), the GUI workflow suite (#33) and the
  feature scorecard (#34). The first two are landing gates whose floors only ever go up, enforced
  locally by whoever lands a change on `main`; the scorecard measures progress and gates nothing.
  See [`features/README.md`](./features/README.md) for the catalog those tools read, and
  DESIGN.md §6.5 for how the layers fit together. All three start in M1 and grow with every
  milestone after it.
- **The landing scripts** (`tools/scripts/gate.sh`, `land.sh`, `gui-ratchet.sh`, `mutate.sh`) are
  the local gate; CLAUDE.md §Build cites them (Hardening, #165).
...
2. **Implement**: code and tests on a branch, landed directly on `main` after the local gate
   (no pull requests since 2026-09-22; CLAUDE.md).
3. **Review**: for `code-data` issues, the review checks every encoded row against the primary
   source.
...
- **#4 and #12 can be designed in parallel now.** #12 is M4 work, but designing it early means
  ...
  need from them.
- **CI (#1) is in place**, and the coverage and GUI ratchets guard every landing on `main`. The
  dependency-license gate (#2) is deferred: each new package's license is checked by hand.
- **Connecticut 2022 (IRC 2021 as amended) is encoded first** (M4/M5) — a second pack is
  what makes code locking and recompute something you can watch happen rather than something the
  design asserts. Connecticut 2026 (IRC 2024 base) is not yet in force. Massachusetts and
  Pennsylvania come after that (DESIGN.md §8, §11).
